...
export default class SpaceAndTimeSDK implements Config {
    ...
    async createTable(sqlText: string, biscuitTokens: string[] = [], originApp: string = ""): Promise<[HttpSuccess | null, null | HttpError]> {
        try {
            ...
            const urlParts: string[] = this.baseUrl.split("/");
            urlParts[urlParts.length - 1] = "v1";
            const newBaseUrl = urlParts.join("/");
            ...
            const response: AxiosResponse<HttpSuccess> = await axios.post(`${newBaseUrl}/sql/ddl`, payload, config);
            return [ response.data, null ];
        }
        catch(error){
            const httpError: HttpError = {
    ...
    async executeQuery(query: any, biscuits: string[] = []): Promise<[HttpSuccess | null, null | HttpError]> {
        try {
            const session = await this.checkSession();
            if (!session) {
                const error: HttpError = {
            ...
            const response: AxiosResponse<HttpSuccess> = await axios.post(`${this.baseUrl}/v1/sql/dql`, data, config);
            return [ response.data, null ];
        }
        catch(error){
            const httpError: HttpError = {
    ...
    async executeTamperproofQuery(sqlText: string, biscuits: string[] = []): Promise<[HttpSuccess | null, null | HttpError]> {
        try {
            ...
            const response: AxiosResponse<HttpSuccess> = await axios.post(`${newBaseUrl}/sql/tamperproof-query`, data, config);
            return [ response.data, null ];
        }
        catch(error){
            console.log(error);
